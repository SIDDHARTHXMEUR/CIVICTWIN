  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
        ...
        {checkpoints.map((cp) => {
          const badgeColor =
            cp.riskLabel === 'Critical' || cp.riskLabel === 'Severe'
              ? { bg: isDark ? '#2b1010' : '#fef2f2', border: '#ea3b1b', text: '#ea3b1b' }
              : { bg: isDark ? '#201809' : '#fffbeb', border: '#f59e0b', text: '#b45309' };
            ...
            >
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  ...
                  >
                    {cp.riskLabel} ({cp.riskPercent}%)
                  </span>
                </div>
                <span style={{ fontSize: '8.5px', color: isDark ? '#cbd5e1' : '#334155', fontWeight: 600 }}>
                  {cp.affectedPopulation.toLocaleString()} pop · {cp.impactRadiusMeters}m
                </span>
              </div>
              ...
              <div
                style={{
                  width: '100%',
                  height: '3px',
                  backgroundColor: isDark ? '#2a2f3d' : '#e2e8f0',
                  borderRadius: '0px',
                  overflow: 'hidden',
                }}
              >
              ...
              >
                <span>Estimated Disruption:</span>
                <strong style={{ color: isDark ? '#cbd5e1' : '#1e293b', whiteSpace: 'nowrap' }}>
                  {formatIndianCurrency(cp.projectedCostInr)}
                </strong>
              </div>
            </div>
          );
